...
| End-to-end pytest suite | ✅ | [tests/test_e2e.py](../tests/test_e2e.py), [tests/test_watcher.py](../tests/test_watcher.py), [tests/conftest.py](../tests/conftest.py) | **36 tests** total: 30 Watcher allowlist unit tests (deterministic, 5 s), 4 parametrized HTTP e2e, 1 approve-flow integration, 1 direct-graph retrieval check. All passing in ~44 s. `pytest` added to [requirements.txt](../requirements.txt). |
...
- **10 / 10 Milestone 3 rows fully delivered (`✅`)**, including:
  ...
  - The new rubric-scored judge with N=3 majority vote, bracketing true groundedness between the binary lower bound (60 %) and the 8B rubric upper estimate (90 %).
